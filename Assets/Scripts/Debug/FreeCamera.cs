...
namespace UnityEngine.Rendering
{
    ...
    public class FreeCamera : MonoBehaviour
    {
        ...
        /// </summary>
        public float m_LookSpeedController = 30f;
        /// <summary>
        ...
        /// </summary>
        public float m_MoveSpeedIncrement = 0.2f;
        ...
        public Light lght;
        public OVRGrabbable seaScooter;
        public OVRGrabbable scanner;
        public OVRGrabbable flashlight;
        ...
        bool leftShiftLight, leftShift, fire1;

        void UpdateInputs()
        {
            inputRotateAxisX = 0.0f;
            inputRotateAxisY = 0.0f;
            leftShiftLight = false;
            fire1 = false;
        ...
        void Update()
        {
            ...
            if (flashlight.isGrabbed) lght.enabled = true;
            else lght.enabled = false;

            if (scanner.isGrabbed) scanner.GetComponent<Scan>().scanning = true;
            else scanner.GetComponent<Scan>().scanning = false;

            if (seaScooter.isGrabbed == true)
            {
                // Suivi de la caméra via VR  (suivi d'une seule main à la fois) // 
                // Lien avec main droite

                if (seaScooter.grabbedBy.gameObject.name == "CustomHandRight")
                    transform.Translate(oVRCameraRig.rightHandAnchor.forward * Time.deltaTime * 3);

                // Lien avec main gauche 
                else transform.Translate(oVRCameraRig.leftHandAnchor.forward * Time.deltaTime * 3);
