Shader "MyEffects/WaterEffect"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Texture1D _WaterHeightMap;
            //SAMPLER(sampler_WaterHeightMap);
            SamplerState sampler_WaterHeightMap
            {
            ...
            v2f vert(appdata v)
            {
                ...
                v.vertex.z = 0;
                o.vertex = TransformObjectToHClip(v.vertex.xyz); // TransformObjectToHClip
                o.uv = v.uv;
                return o;
            }
            ...
            float heightMap(float2 pos)
            {
                ...
                return _WaterHeightMap.SampleLevel(sampler_WaterHeightMap, float4(uv, 0, 0), 0).r; 
            }
            ...
            /*float Linear01Depth(float InDepth, float NearClip, float FarClip){
            ...
            {
                return _zNear * _zFar / (_zFar + depthValue * (_zNear - _zFar));
            }

            float4 frag(v2f i) : SV_Target
            {
                //float depth = linearize_depth(tex2D(_CameraDepthTexture, i.uv).r) * _ProjectionParams.z;
                
                float depth = linearize_depth( _CameraDepthTexture.Sample(sample_CameraDepthTexture, i.uv).r) * _ProjectionParams.z;
                float flatDepth = 1.0f / length(i.ray); // to convert a spherical euclidian depth to a z buffer distance
                ...
                float delta = max(.05f, planeDepth * .05f);
                ...
                float3 planeNorm = normalize(float3(heightDx, 3.f, heightDy));
                ...
                // TODO
                //half4 skyData = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, bouncedSkyboxRay); // <- texCube
                half3 bouncedSkyboxColor = half3(1, 1, 1);// half3 bouncedSkyboxColor = DecodeHDR( skyData, unity_SpecCube0_HDR); // UNITY_SAMPLE_TEXCUBE
                //SAMPLE_TEXTURECUBE(unity_SpecCube0, sampler_StarCubeMap, starUVW).rgb;
                ...
                float t = volumeBegin + _StepSize * _BlueNoise.Sample(sampler_BlueNoise, i.uv * _BlueNoiseToSceen).r;
                ...
                if (rayPos.y > _WaterHeight)
                {
                    ...
                    if (planeDepth > 0.0f & planeDepth < depth)
                    {
                        col += att * planeFresnel * bouncedSkyboxColor;
                        att *= 1.0f - planeFresnel;
                    }
                    // volume after
                    col += volCol;
                    att *= volAtt;
                ...
                else
                {
                    ...
                    if (planeDepth > 0.0f & planeDepth < depth)
                    {
                        col += att * (1.0f - planeFresnel) * bouncedSkyboxColor;
                        att *= 0.0f;
                    }
                }
                ...
                col += _MainTex.Sample(sampler_MainTex, i.uv).rgb * att;
                col = (col * (2.51f * col + .03f)) / (col * (2.43f * col + .59f) + .14f); // quick filmic curve
